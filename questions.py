...
This file is data: the questions annotators are asked and the scales they use.
The code that renders, stores and exports them is in annotation.py and
export_annotations.py, and does not need editing to change a question.

Three commands, none of which needs a database or a browser:

    python questions.py --show dond      # what an annotator will see
    python questions.py --check          # validate every entry before it ships
    python questions.py --markdown       # regenerate question_set.md

--check is the safety net. The only other check is an assertion that fires
when an annotator presses Submit, which is far too late.

Never hand-edit question_set.md. It is generated by --markdown, and a
hand-written copy goes stale without anyone noticing.


HOW AN ENTRY IS SHAPED
----------------------

BESPOKE_QUESTIONS maps a game family (the folder name under games/) to what
that game overrides. Anything left out falls back to the Shared Core:
GENERIC_Q1, GENERIC_Q2, SC_TICKS, and the conditional GENERIC_Q3 / SC_TICK_D
pair. A family with no entry here uses the Shared Core only.

    "dond": {
        "roles": {                        # keyed by role name in the transcript
            "DealOrNoDealPlayer": {
                "q1": "generic",          # "generic" -> the shared question
                "q2": None,               # None      -> not asked of this role
                # or a (label_md, choices) tuple -> this game's own question
            },
        },
        "flags": SC_TICKS + [...],        # None -> the shared ticks
                                          # []   -> no ticks at all
        "reasoning_clarity": True,        # show Q3 + the explanation tick
        "whole_game": [...],              # asked once, on the verdict screen
    }

Choices are [(display, stored_value), ...]. Use the helpers: _scale4([...])
for a 1-4 scale, _scaleN(7, {1: "low", 7: "high"}) for anything else. A
display of "3\nGood" becomes a numbered button; a bare "N/A" stores "NA".

Rules to keep:

  * Whole-game entries are (id, label_md, choices). The id is what the answer
    is stored under, so never rename one after data exists. Reordering is safe.
  * "bolt_ons" adds a question to every turn of a role; "bolt_ons_first_turn"
    adds one to that role's first turn only.
  * Every label is "**Title**\n\nThe question?". plain_label() splits on that
    shape to build the screen-reader name.


CHANGING A QUESTION MID-STUDY
-----------------------------

Every saved annotation stores a fingerprint of the question set it was
collected under. Editing a game that already has rows does not corrupt them,
but it splits that game across two question sets, and an annotator working on
it is asked to start that transcript again. Adding a game with no data yet
costs nothing.
"""
...
# The shared per-turn questions. Same (label_md, choices) shape as a bespoke
# entry, so the render loop and the export read both the same way.
GENERIC_Q1 = (
...
# Constants so a game can extend the shared ticks instead of copying them; a
# copy would drift as soon as one is reworded. SC_TICK_D goes with Q3 and shows
# only where the game asks the model to explain its move.
SC_TICKS = [
...
BESPOKE_QUESTIONS = {
    ...
    "codenames": {
        ...
        "whole_game": [
            (
                "clue_safety_overall",
                # 1-4, not the 1-7 other games use, so it matches the per-turn
                # safety question and the two can be compared directly.
    ...
    "dond": {
        # Shared ticks plus the explanation pair, and one hazard the core
        # cannot express: item values are private, and weaker models say them
        # out loud in the open chat.
        "flags": SC_TICKS + [
            SC_TICK_D,
        ...
        "whole_game": [
            ...
            (
                "negotiation_quality",
                # Says nothing about whether the split was fair: the annotator
                # never sees both players' secret values. Kept separate from
    ...
    "imagegame": {
        # This game's failure mode is an instruction that never lands on the
        # grid, so it needs its own ticks. The first one overlaps the Follower's
        # Q1 on purpose: Q1 grades how well the grid matched, the tick records
        # the plain "it did not move" event the Giver should notice.
        "flags": [
        ...
        # Every game keeps the generic G1/G2 pair, so there is one measure that
        # can be compared across games.
        "roles": {
            "Instruction Giver": {
                "q1": (
        ...
        "whole_game": [
            ...
            # One overall question only. The Follower's per-turn Q1 already
            # covers execution.
        ],
    },

    # 13 families from the test_newgames bundle. cryptolect, ta_blackjack and
    # the data-less bbh/cladder/mmlu_pro placeholders are excluded on purpose.
...
# wordle-crazy scrambles the feedback colours and defines the key only in that
# episode's rules, so this asks whether the model applied that key correctly.
_WORDLE_CRAZY_Q1 = (
...
BESPOKE_QUESTIONS.update({
    ...
    # The study variant, so it uses the shared wording. The other wordle-crazy
    # entries are outside the study and keep their own questions.
    "wordle-crazy_withclue": {
        ...
        "roles": {
            "WordGuesser": {
                ...
                # Opening guess only. From the second guess on there is letter
                # feedback too, so "did it use the clue" stops being clean.
                "bolt_ons_first_turn": [
                    (
...
BESPOKE_QUESTIONS.update({
    "guesswhat": {
        # Uses the shared Q1/Q2 wording; anything game-specific goes in the
        # whole-game question instead.
        "roles": {
    ...
    "referencegame": {
        ...
        "whole_game": [
            (
                ...
                # The per-turn questions score each side alone, but the common
                # failure is a clear description followed by a wrong pick.
                "**Whole game — Taken together, how well did the Giver's "
                "description and the Follower's pick work as a pair?**",
                # 4 is anchored too: it is a specific case, not the midpoint of
                # "bad to good" that annotators would otherwise assume.
                _scaleN(7, {
...
# CLI. annotation.py is imported inside these functions, not at the top: it
# imports this file, so a module-level import would be a cycle. They call the
# app's own functions rather than copying the render rules, so a preview can
# never disagree with what an annotator is shown.
...
def _check_choices(where, choices, err, warn):
    ...
    # All-numbered is a scale; all-plain is a set of named options. Both fine.
    # Mixing them is the problem: half the options imply an order.
    ...
    # A scale must run 1, 2, 3, … A gap means the stored number stops matching
    # the label.
...
def _check_bolt_ons(where, entries, keys_seen, err, warn):
    for i, e in enumerate(entries or []):
        ...
        if not isinstance(key, str) or not key.isidentifier():
            # The key becomes an "extra:<key>" column in the export.
...
def check(verbose=False):
    ...
    fams = _families()
    # Families with no transcripts under the current GAMES_DIR. Normal for a
    # curated tree, so they are summarised on one line.
    unreachable = []
    ...
    for game in sorted(BESPOKE_QUESTIONS):
        ...
        for role, cfg in roles.items():
            ...
            # A role name no transcript uses means the entry is skipped and the
            # game quietly falls back to the Shared Core.
            ...
            if not isinstance(wg, list):
                ...
                for i, e in enumerate(wg):
                    ...
                    if not (isinstance(e, (list, tuple)) and len(e) in (2, 3)):
                        ...
                        continue
                    if len(e) == 2:
                        # _wg_id hashes the text, so rewording the question
                        # makes it a different question to the export.
    ...
    # with what the render loop would wire up.
    checked = 0
...
def markdown():
    ...
    Generated rather than hand-written, so it cannot go stale.
    """
    import annotation
    fams = _families()
    out = [
