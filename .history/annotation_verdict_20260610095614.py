...
_COHERENCE_DESC = (
    "**1** — Each move seems disconnected from the last — no consistent logic across turns.\n\n"
    "**2** — Had a plan but kept following it even when feedback clearly showed it was not working.\n\n"
    "**3** —  Maintained a clear goal and adjusted its approach when something was not working.\n\n"
    "**4** — Built every new piece of information into its plan smoothly."
)
...
def _save_verdict(coherence, overall, comment):
    if not coherence:
        return "Please rate Strategic Coherence before submitting."
    if not overall:
        return "Please give an Overall Rating before submitting."

    try:
        with open(OUTPUT_PATH) as f:
            saved = json.load(f)
    except FileNotFoundError:
        return "Turn annotations not found. Please complete Step 1 first."
