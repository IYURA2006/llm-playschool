...
def _coh_select(chosen):
    """Update column highlight + button variant for each card."""
    col_updates = [
        gr.update(elem_classes=["coh-col", "coh-col-sel"] if v == chosen else ["coh-col"])
    ...
    return (*col_updates, *btn_updates, chosen)
...
def _save_verdict(coherence, overall, comment):
    ...
    if not coherence or not overall:
        msg = "⚠️ Please fill in the highlighted fields before submitting."
        return (msg, gr.update(elem_classes=o_cls))
...
def build(welcome_page, annotation_page, verdict_page):
    with verdict_page:
        ...
        # G1: Strategic Coherence — 4 columns, each with Markdown + select button
