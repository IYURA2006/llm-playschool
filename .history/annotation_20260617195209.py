...
DATA_PATH = os.path.join(_dir, "interactions", "interactions-6.json")
...
def _build_transcript_html(current_idx: int) -> str:
    parts = [] 
    turn_counter = 0 #ai response turn index
    ...
    for round_msgs in (_data["turns"]):
        for msg in round_msgs:
            sender = msg["from"] #who sent it
            action = msg["action"] #the action dict
            atype = action["type"] #e.g. "correct guess", "response", "context"
            label = action.get("label", "") 
            content = action["content"] #actual text

            if sender == "GM":
                # Some game formats include GM metadata messages whose content is
                # a dict/list (action info, goal status, etc.) — not meant for display.
                ...
                if label == "context":
                    parts.append(
                        f'<div class="gm-msg">'
                        f'<span class="gm-tag">GM</span> {html.escape(content)}'
                        f'</div>'
                    )
                    ...
                    if content == "end game":
                        ...
                        parts.append(
                            f'<div class="correct-msg">✅ Correct: <strong>{html.escape(content)}</strong></div>'
                        )
            elif label == "response":
                #card user looking now
                active = turn_counter == current_idx

    
                if active:
                    card_cls = "turn-card active-turn"
                else:
                    card_cls = "turn-card"
                ...
                parts.append(
                    ...
                    f'{html.escape(sender.upper())} (AI)&nbsp;&nbsp; · &nbsp;&nbsp;TURN {turn_counter + 1} OF {N_TURNS}'
                    f'</div>'
                    f'<div class="card-body">{html.escape(content)}</div>'
                    f'</div>'
                )
...
def _submit(*vals):
    ...
    #vals arrive as a flat list of length 5*n: [q1v, q2v, q3v, fv, cv]
...
def build(welcome_page, annotation_page, verdict_page):
    with annotation_page:
        ...
        submit_btn.click(
            fn=_submit,
            #unpack the list of lists into a flat list for the function
