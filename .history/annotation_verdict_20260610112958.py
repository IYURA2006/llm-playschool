...
_COH_OK  = ["coh-btn"]
_COH_ERR = ["coh-btn", "coh-btn-error"]


def _coh_select(chosen):
    """Return button variant updates + the chosen value."""
    return (
        *[gr.update(variant="primary" if v == chosen else "secondary",
                    elem_classes=_COH_OK)
          for v, _, _ in _COHERENCE],
        chosen,
    )
...
def _save_verdict(coherence, overall, comment):
    ...
    if not coherence or not overall:
        coh_updates = [gr.update(elem_classes=_COH_ERR if c_err else _COH_OK)
                       for _ in _COHERENCE]
        return ("⚠️ Please fill in the highlighted fields before submitting.",
                *coh_updates, gr.update(elem_classes=o_cls))
    ...
    try:
        ...
        return ("⚠️ Turn annotations not found. Please complete Step 1 first.",
                *[gr.update(elem_classes=_COH_OK) for _ in _COHERENCE],
...
def build(welcome_page, annotation_page, verdict_page):
    with verdict_page:
        ...
        # G1: Strategic Coherence — 4 gr.Button cards in a row
        ...
        # ── EVENT WIRING ──────────────────────────────────────────────
        for i, (val, _, _) in enumerate(_COHERENCE):
            coh_btns[i].click(
                fn=lambda v=val: _coh_select(v),
                outputs=[*coh_btns, coherence],
            )

        submit_btn.click(
            fn=_save_verdict,
            inputs=[coherence, overall, comment],
            outputs=[status, *coh_btns, overall],
        )
