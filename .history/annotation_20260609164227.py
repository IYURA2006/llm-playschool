...
def _submit(turn_idx, anns, q1v, q2v, q3v, fv, cv):
    anns = dict(anns)
    anns[turn_idx] = {"q1": q1v, "q2": q2v, "q3": q3v, "flags": fv or [], "comment": cv or ""}
    missing = [i + 1 for i, a in anns.items() if not a["q1"] or not a["q2"]]
    if missing:
        return anns, f"⚠️ Missing required ratings for turns: {missing}"
    turns_out = []
    ...
    return anns, "✅ Saved to interactions/annotations-4.json"
...
def build(welcome_page, annotation_page):
    with annotation_page:
        ...
        with gr.Row(elem_classes=["annot-topnav"]):
            ...
            gr.HTML('<div class="nav-timer">00:00</div>')
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=2, elem_id="annot-col"):
                ...
                gr.HTML(
                    '<div class="ql"><span class="qn">Q1</span>'
                    '<span class="qt"> — Prior Information Use</span></div>'
                    '<div class="qd">Did the AI correctly use information established in earlier turns?</div>'
                )
                q1 = gr.Radio(
                    choices=[("1\nNone", "1"), ("2\nPartial", "2"), ("3\nGood", "3"), ("4\nExcellent", "4")],
                ...
                gr.HTML(
                    '<div class="ql"><span class="qn">Q2</span>'
                    '<span class="qt"> — Strategic Logic</span></div>'
                    '<div class="qd">Regardless of constraints, did this move make strategic sense?</div>'
                )
                q2 = gr.Radio(
                    choices=[("1\nNonsensical", "1"), ("2\nPoor", "2"), ("3\nReasonable", "3"), ("4\nStrong", "4")],
                    label="", show_label=False,
                ...
                gr.HTML(
                    '<div class="ql"><span class="qn">Q3</span>'
                    '<span class="qt"> — Reasoning Clarity</span>'
                    '<span class="qd">“How clearly does the AI explain what it is doing and why in this turn?”</span></div>'
                    '<span class="cond-tag">conditional</span>'
                )
                q3 = gr.Radio(
