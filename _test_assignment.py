...
def test_basic_batch_and_idempotency():
    ...
    check("batch is a single model",
          len({study_set.DIMENSIONS[i["game"]]["model_id"] for i in playlist1}) == 1)
...
def test_pick_batch_is_pure():
    ...
    def pick(counts, **kw):
        return assignment._pick_batch(counts, 3, rng=random.Random(0),
                                      batch_members=BM, batch_template=BT, **kw)
    ...
    check("template exclusion is honoured",
          pick(zero, exclude_templates={"A", "B"}).batch_id == "C__m")
    check("all templates excluded -> ANNOTATOR_EXHAUSTED",
          pick(zero, exclude_templates={"A", "B", "C"}).reason
...
def test_exhausted_is_distinct_from_no_tasks():
    ...
    try:
        assignment.MAX_BATCHES = 99          # let templates, not the cap, stop them
        seen = 0
        while True:
            playlist, err = assignment.build_playlist_for(pid)
            if err:
            ...
            seen += 1
            complete_session(pid)
        check(f"PID exhausted all {len(study_set.TEMPLATE_BATCHES)} templates",
              seen == len(study_set.TEMPLATE_BATCHES))
